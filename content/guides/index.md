---
title: Guides
description: Guides for Orlo Open Core, Orlo Platform, and operational AI governance
---

# Guides

Use these guides to understand Orlo, choose the right product surface, and connect AI engineering work to operational governance.

## Start with

- [Getting Started](/guides/getting-started)
- [What Is Orlo?](/guides/what-is-orlo)
- [Open Core vs Platform](/guides/open-core-vs-platform)
- [Who Orlo Is For](/guides/who-orlo-is-for)
- [Governance](/governance)

## Choose by Job

| Job | Start Here |
| --- | --- |
| Understand the category | [What Is Orlo?](/guides/what-is-orlo) |
| Decide between public packages and the managed product | [Open Core vs Platform](/guides/open-core-vs-platform) |
| Understand Orlo beside LLM gateways | [Orlo And LLM Gateways](/guides/architecture/orlo-and-llm-gateways) |
| Run the managed production loop | [Platform Overview](/guides/platform/platform-overview) |
| Operate Orlo through the dashboard | [Dashboard Surfaces](/guides/platform/dashboard) |
| Govern internal AI use | [Governance](/governance) |
| Integrate with the hosted API | [Platform API](/platform-api) |
| Use reusable public packages | [Open Core](/guides/open-core) |

## Open Core

- [Open Core Overview](/guides/open-core)
- [Validation Quickstart](/guides/open-core/validation-quickstart)
- [Runtime Adapters Quickstart](/guides/open-core/runtime-adapters-quickstart)
- [Studio Quickstart](/guides/open-core/studio-quickstart)
- [Agent SDK Quickstart](/guides/open-core/agent-sdk-quickstart)

## Platform

- [Platform Overview](/guides/platform/platform-overview)
- [Dashboard Surfaces](/guides/platform/dashboard)
- [Platform Quickstarts](/guides/platform/quickstarts)
- [Authentication and Org Model](/guides/platform/authentication-and-org-model)
- [Evaluation to Deployment Loop](/guides/platform/evaluation-to-deployment-loop)
- [Retrieval and Documents](/guides/platform/retrieval-and-documents)
- [Feedback and Promotion](/guides/platform/feedback-and-promotion)
- [Agent Governance Overview](/guides/platform/agent-governance-overview)

## Governance

- [Governance Overview](/governance)
- [Internal AI Operating Model](/governance/internal-ai-operating-model)
- [Operational Evidence](/governance/operational-evidence)
- [Runtime Controls](/governance/runtime-controls)
- [AI Act Readiness Evidence](/governance/ai-act-readiness-evidence)

## Architecture and Adoption

- [Architecture](/guides/architecture)
- [Orlo And LLM Gateways](/guides/architecture/orlo-and-llm-gateways)
- [Adoption](/guides/adoption)
